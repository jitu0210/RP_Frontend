  ...
  //   const bgClass = isDarkMode ? "bg-gray-900" : "bg-gray-300";//////100 = 300
  ...
  //             className={`${statusCardBg} p-3 rounded border ${cardBorderClass}`}
  //           >
  //             <div className={`text-xs ${statusCardText}`}>ANALOG</div>
  //             <div
    ...
    buttons: [
      {
        ...
        color: "gray",
        directionTag: "b1_s1_available",
        type: "direction", // Changed from "arm" to "direction"
      },
      {
        label: "BusA to Src2",
        endpoint: "/api/v1/bus1/source2",
    ...
    buttons: [
      {
        ...
        endpoint: "/api/v1/breaker/operate",
        color: "orange",
        type: "breaker", // Changed from "execute" to "breaker"
      },
    ],
  },
];
...
export default function ModbusControlPanel() {
  ...
  const addLog = (msg, type = "INFO") => {
    const allowedTypes = [
      "CMD",
      "DIRECTION", // Changed from "ARMED" to "DIRECTION"
      "BREAKER",   // New type for breaker operations
      "SUCCESS",
      "ERROR",
      "MODE_CHANGE",
      "STATUS_CHANGE",
  ...
  return (
    <div className={`min-h-screen ${bgClass} ${textClass} p-3`}>
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-4 gap-2  mb-4">
          <div
            className={`${statusCardBg} p-3 overflow-hidden rounded border ${cardBorderClass}`}
          >
            <div className={`text-xs ${statusCardText}`}>ACTIVE COMMAND</div>
            <div
              className={`text-sm font-bold truncate ${statusValueText(activeCommand, "text-blue-400", "text-blue-600")}`}
          ...
          <div
            className={`${statusCardBg} p-3 rounded border ${cardBorderClass}`}
          >
            <div className={`text-xs ${statusCardText}`}>SYSTEM</div>
            <div
              className={`text-sm font-bold ${loading ? "text-yellow-500" : "text-green-500"}`}
            >
              {loading ? "BUSY" : "READY"}
            </div>
          </div>
        ...
        <div className="space-y-4">
          {actions.map((section, idx) => (
            ...
            >
                  ...
                  return (
                    <div key={i} className="relative">
                      <button
                        ...
                        className={`relative w-full p-3 rounded border text-sm font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${
                          isDirection 
                            ? getDirectionClass(directionAvailable) 
                            : section.title === "Remote Mode" 
                              ? getModeActiveClass(modeActive) 
                              : colorClasses[btn.color]
                        } ${isActive ? "ring-2 ring-offset-2 ring-offset-gray-900 ring-blue-500" : ""}`}
                      >
                        <div className="flex items-center justify-between">
                          <span>{btn.label}</span>
              ...
              {/* Removed the ARMED direction display section */}
            </div>
          ))}
        </div>
